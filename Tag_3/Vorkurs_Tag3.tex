...
arrayName[i]
	\end{lstlisting}
	\color{red} Achtung: \color{black} Der Index geht von $0$ bis $n-1$!
	}
...
do {
	...
	\vspace{2em}
	Anders als While-Schleifen wird eine Do-While-Schleife immer \emph{mindestens einmal} durchlaufen.
\end{frame}
...
\end{lstlisting}
	...
	\begin{alertblock}{Achtung!}
		In diesem Beispiel werden die Anweisungen in der While-Schleife \emph{gar nicht} ausgeführt, die in der Do-While-Schleife \emph{einmal}.
	\end{alertblock}
\end{frame}
...
\subsection{Welche Schleife?}
\begin{frame}
	\frametitle{Wann welche Schleife?}
	\begin{itemize}
		\item For-Schleifen wenn die \emph{Anzahl der Durchläufe} im voraus  \emph{bekannt} ist
			\begin{itemize}
				\item (Wenn man einen Zähler braucht)
			\end{itemize}
		\vspace{1em}
		\item While/Do-While schleifen in anderen Fällen
		\begin{itemize}
				\item (Wenn man keinen Zähler braucht)
			\end{itemize}
	\end{itemize}
\end{frame}
...
\begin{frame}
	...
	\end{center}
	\begin{itemize}
		\item{Original von ? 2013 von Felix Bürkle}
		\item{Überarbeitet 2015 von Eva Wolkwitz}
	\end{itemize}
\end{frame}
